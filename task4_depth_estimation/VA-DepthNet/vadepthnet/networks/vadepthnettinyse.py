...
class MetricLayer(nn.Module):
    def __init__(self, c):
        ...
        self.ln = nn.Sequential(
                nn.Linear(c, c//4),
                nn.LeakyReLU(),
                nn.Linear(c//4, c)) # output original channel length
...
class VADepthNetTinySE(nn.Module):
    def __init__(self, pretrained=None, max_depth=10.0, prior_mean=1.54, si_lambda=0.85, img_size=(480, 640)):
        ...
        self.fclayer = nn.Conv2d(768, 1, kernel_size=1, padding=0)
    ...
    def forward(self, x, gts=None):
        ...
        # Upsample
        x5_u = F.interpolate(x5, scale_factor=32, mode='bilinear', align_corners=True)
        metric = self.mlayer(x5_u)
        ...
        # metric * (depth_map + feature_map)
        d = metric * (self.outc(d) + x5_u)
        
        d_final = self.fclayer(d)
        
        outs['scale_1'] = d_final
