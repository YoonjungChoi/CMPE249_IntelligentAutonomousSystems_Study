...
from networks.vadepthnetcnn import VADepthNetCNN


parser = argparse.ArgumentParser(description='VADepthNetCNN PyTorch implementation.', fromfile_prefix_chars='@')
parser.convert_arg_line_to_args = convert_arg_line_to_args

parser.add_argument('--mode',                      type=str,   help='train or test', default='train')
parser.add_argument('--model_name',                type=str,   help='model name', default='vadepthnetcnn')
...
def main_worker(gpu, ngpus_per_node, args):
    print("LOGYJ gpu ", gpu, " , ngpus_per_node ", ngpus_per_node)
    args.gpu = gpu
    ...
    model = VADepthNetCNN(backbone_name=args.pretrain,
    ...
    if args.checkpoint_path != '':
        if os.path.isfile(args.checkpoint_path):
            ...
            print("LOG checkpoint", checkpoint.keys())
            global_step = checkpoint['global_step']
            print("LOG global_step", global_step)
            model.load_state_dict(checkpoint['model'])
            optimizer.load_state_dict(checkpoint['optimizer'])
            if not args.retrain:
                print("LOG retrain is not started from zero step")
                try:
                    global_step = checkpoint['global_step']
    ...
    print("=== steps_per_epoch {d}, num_total_steps {d} ===")
...
def main():
    ...
    if args.multiprocessing_distributed:
        print("LOGYJ mp.spawn")
        args.world_size = ngpus_per_node * args.world_size
        mp.spawn(main_worker, nprocs=ngpus_per_node, args=(ngpus_per_node, args))
    else:
        print("LOGYJ main_worker")
